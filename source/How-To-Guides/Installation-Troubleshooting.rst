...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
While following the :doc:`Creating a workspace </Tutorials/Beginner-Client-Libraries/Creating-A-Workspace/index>` tutorial, you might encounter the following error stating that ``rosdep`` failes to install Qt5.
